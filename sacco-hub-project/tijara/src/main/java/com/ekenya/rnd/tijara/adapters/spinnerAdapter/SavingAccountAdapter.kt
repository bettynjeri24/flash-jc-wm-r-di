...
class SavingAccountAdapter(context: Context, list: List<SavingAccountData>) : ArrayAdapter<SavingAccountData>(context, 0, list) {
    var name1=""
    ...
    private fun createView(position: Int, recycledView: View?, parent: ViewGroup): View {
        ...
        val accNumber=name?.accountNo?.replace("(?<=.{3}).(?=.{3})".toRegex(),"*")
            val formatNmae= name?.accountName?.let { camelCase(it) }
           /* val splited: List<String> = formatNmae?.split("\\s".toRegex())
            ...
            view.tv_gender.text= "$formatNmae-A/C#$accNumber"

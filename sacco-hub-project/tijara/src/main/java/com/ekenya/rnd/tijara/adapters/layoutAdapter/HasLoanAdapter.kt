...
class HasLoanAdapter (val items:ArrayList<UserProfileList>): RecyclerView.Adapter<HasLoanAdapter.LoanOptionViewHolder>() {
    ...
    override fun onBindViewHolder(holder: LoanOptionViewHolder, position: Int) {
        ...
        holder.itemView.cardLoan.setOnClickListener {
            when(position){
                0 ->{
                    it.findNavController().navigate(R.id.loanRepaymentFragment)
                }
               1 ->{

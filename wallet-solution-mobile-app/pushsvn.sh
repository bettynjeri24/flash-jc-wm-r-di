...
alias agent='C:/KiuwanLocalAnalyzer/KiuwanLocalAnalyzer/bin/agent.cmd'
...
# get credentials from windows environmentsvn commit -m "$COMMENT_ARGUMENT" --username=$SVN_USERNAME --password=$SVN_PASSWORD
...
folderName="$(PWD | sed 's!.*/!!')"
  ...
  agent -n "Kotlin" -s "$CI_PROJECT_DIR/onboarding/src/main/java/com/ekenya/rnd/onboarding" -l "devops_Mari Wallet" -c --user $KIUWAN_USER --pass $KIUWAN_PASSWD
